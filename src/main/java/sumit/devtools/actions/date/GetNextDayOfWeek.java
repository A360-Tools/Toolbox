 ...
 */
@BotCommand
@CommandPkg(label = "Get Next Day Of Week",
    description = "Gets next date of particular day of week",
    icon = "Calendar.svg",
    name = "getNextDayOfWeek",
    group_label = "Date",
    node_label = "Get next {{daysOfWeek}} of {{inputDate}} and assign to {{returnTo}}",
    return_label = "Assign next day of week to",
    return_description = "Datetime of the next day of the week",
    return_required = true,
//        allowed_agent_targets = AllowedTarget.HEADLESS,
    return_type = DataType.DATETIME)
public class GetNextDayOfWeek {
  ...
  public static DateTimeValue action(
    ...
    try {
      ...
      if (includeInputIfMatch) {
        resultDT = inputDate.with(TemporalAdjusters.nextOrSame(dayOfWeek));
      } else {
        resultDT = inputDate.with(TemporalAdjusters.next(dayOfWeek));
      }

      result.set(resultDT.truncatedTo(ChronoUnit.DAYS));
      return result;
    } catch (Exception e) {
      throw new BotCommandException("Failed to calculate the next day of week: " + e.getMessage(),
          e);
    }
