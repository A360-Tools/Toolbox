 ...
 */
@BotCommand
@CommandPkg(label = "Get Next Working Date",
    description = "Gets next working date (Monday to Friday are considered working days)",
    icon = "Calendar.svg",
    name = "getNextWorkDate",
    group_label = "Date",
    node_label = "Get next working date from {{inputDate}} and assign to {{returnTo}}",
    return_label = "Assign next working date to",
    return_description = "Datetime of the next working day",
    return_required = true,
//        allowed_agent_targets = AllowedTarget.HEADLESS,
    return_type = DataType.DATETIME)
public class GetNextWorkDate {
  ...
  public static DateTimeValue action(
    ...
    try {
      ...
      throw new BotCommandException("Failed to calculate the next work date: " + e.getMessage(), e);
    }
  }
